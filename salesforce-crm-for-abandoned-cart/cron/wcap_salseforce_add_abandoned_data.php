...
class Wcap_Salesforce_CRM_Add_Cron_Data {         
    ...
    public static function wcap_add_salesforce_abandoned_cart_data() {
        ...
        global $wpdb;
        global $woocommerce;
        $wcap_automatic_add_to_sf  = get_option ( 'wcap_add_automatically_to_salseforce_crm' );
        if ( isset( $wcap_automatic_add_to_sf ) && 'on' == $wcap_automatic_add_to_sf ){
            ...
            $get_last_checked_id  = get_option ( 'wcap_salseforce_last_id_checked' );
            
           //wp_mail ('kiribhavik4@gmail.com' , 'get_last_checked_id', print_r($get_last_checked_id,true));
            //wp_mail ('kiribhavik4@gmail.com' , 'get_last_sf_add_abandoned_cart_id', print_r($get_last_sf_add_abandoned_cart_id,true));
            ...
            if ( '' === $get_last_checked_id || false === $get_last_checked_id || $get_last_checked_id < $get_last_sf_add_abandoned_cart_id ) {
                ...
                //wp_mail ('kiribhavik4@gmail.com' , 'all cart id', print_r($abandoned_cart_ids,true));
                ...
                update_option ( 'wcap_salseforce_last_id_checked' , $get_last_sf_add_abandoned_cart_id ); 
            }
        }
    }
   ...
   public static function wcap_get_last_abandoned_cart_id(){
        ...
        $query_records     = "SELECT id FROM `" . $wpdb->prefix . "ac_abandoned_cart_history` WHERE `id` NOT IN ( SELECT abandoned_cart_id FROM `".$wpdb->prefix."wcap_salseforce_abandoned_cart`) AND recovered_cart = 0 AND `user_id` > 0 AND abandoned_cart_info NOT LIKE '$blank_cart_info_guest' AND abandoned_cart_info NOT LIKE '%$blank_cart_info%' ORDER BY id DESC LIMIT 1";

        //wp_mail ('kiribhavik4@gmail.com' , 'query_records', print_r($query_records,true));
   ...
   public static function wcap_get_carts_for_sf ( $get_last_checked_id ){
        ...
        $wcap_get_all_abandoned_carts = "SELECT id FROM `".$wpdb->prefix."ac_abandoned_cart_history` WHERE `id` NOT IN ( SELECT abandoned_cart_id FROM `".$wpdb->prefix."wcap_salseforce_abandoned_cart`) AND `id` > $get_last_checked_id AND `user_id` > 0 and `recovered_cart` = 0";
    ...
    public static function wcap_add_data_to_sf ( $abandoned_cart_ids_key ) {
        ...
        //wp_mail ('kiribhavik4@gmail.com', 'ad from here', 'ad from here');

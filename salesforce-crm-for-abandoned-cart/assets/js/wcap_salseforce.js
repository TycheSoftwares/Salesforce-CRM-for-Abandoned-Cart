jQuery(function( $ ) {
	...
	$ ( '.button-primary' ).on( 'click', function( e ) {
		if ( $(this).val() == 'Save Salesforce Changes' ) {
			...
			var wcap_sf_token     = document.getElementById('wcap_salseforce_security_token').value;

			if ('' == wcap_sf_token ){
				$( "#wcap_salseforce_security_token_label_error" ).fadeIn();
	            setTimeout( function(){
		            $( "#wcap_salseforce_security_token_label_error" ).fadeOut();
		        },4000);

		        e.preventDefault();
			}
			if ('' == wcap_sf_passwrod ){
				$( "#wcap_salseforce_password_label_error" ).fadeIn();
	            setTimeout( function(){
		            $( "#wcap_salseforce_password_label_error" ).fadeOut();
		        },4000);
			    e.preventDefault();
			}
			if ('' == wcap_sf_user_name ){
				$( "#wcap_salseforce_user_name_label_error" ).fadeIn();
	            setTimeout( function(){
		            $( "#wcap_salseforce_user_name_label_error" ).fadeOut();
		        },4000);
	            e.preventDefault();
			}
		}
	...
	$ ( '.add_single_cart_salesforce' ).on( 'click', function( e ) {
		...
		var data = {
			action                  : 'wcap_add_to_salseforce_crm',
			wcap_abandoned_cart_ids : wcap_selected_id,
			wcap_all                : wcap_all
		};

		$.post( wcap_salseforce_params.ajax_url, data, function( response ) {
			...
			$( ".wcap_salseforce_message_p" ).html( display_message );
            $( "#wcap_salseforce_message" ).fadeIn();
            setTimeout( function(){
            	$( "#wcap_salseforce_message" ).fadeOut();
            },4000);
		});
	});
	...
	$ ( '#doaction' ).on( 'click', function( e ) {
		if ( $( '#bulk-action-selector-top' ).val() == 'wcap_add_salseforce' ) {
				...
				var wcap_parent      = [];
			  	for (var i = 0; i < checkboxes.length; i++) {
			     	...
			     	if ( checkboxes[i].checked ) {
			     		...
			     		var wcap_salesforce = email_check.indexOf("Add to Salseforce CRM");
	...
	$ ( '#doaction2' ).on( 'click', function( e ) {
		if ( $( '#bulk-action-selector-bottom' ).val() == 'wcap_add_salseforce' ) {
	...
	var wcap_user_type_selected = $( 'input[type=radio][name=wcap_salseforce_user_type]:checked' ).val();
	if( wcap_user_type_selected == 'contact' ) {
		$(".wcap_salseforce_lead_company").hide();
            $(".wcap_salseforce_lead_company").closest("tr").hide();
	}

	$('input[type=radio][name=wcap_salseforce_user_type]').change(function() {
        if (this.value == 'lead') {
        	$(".wcap_salseforce_lead_company").closest("tr").fadeIn();
            $(".wcap_salseforce_lead_company").fadeIn();
        }
        else if (this.value == 'contact') {
        	$(".wcap_salseforce_lead_company").fadeOut();
            $(".wcap_salseforce_lead_company").closest("tr").fadeOut();
        }
    });
});